...
module BancoBrasilPayments
  ...
  class BarcodecollectionformsCollectionFormDetailsList
    # Define o código de barras numérico da Guia de Recolhimento.
    attr_accessor :numeric_bar_code

    # Data em que deverá ser efetivado o pagamento (formato: ddmmaaaa). Os pagamentos poderão ser agendados para até 180 dias. Campo Obrigatório.
    attr_accessor :payment_date

    # Valor total das Guias de Recolhimento (formato 0.00). Campo Obrigatório.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BancoBrasilPayments::BarcodecollectionformsCollectionFormDetailsList` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BancoBrasilPayments::BarcodecollectionformsCollectionFormDetailsList`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [numeric_bar_code, payment_date, payment_value, debit_code, your_number, customer_description].hash
    end
